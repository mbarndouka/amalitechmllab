...
def _evaluate(
    model, X_train, X_val, X_test, y_train, y_val, y_test, name: str, log_target: bool = False
) -> dict[str, dict]:
...
def train_decision_tree(X_train, y_train, cfg: dict, cv: int) -> tuple[DecisionTreeRegressor, dict]:
    ...
    logger.info(
        "DecisionTree GridSearchCV  grid_size=%d  cv=%d",
        len(param_grid["max_depth"]) * len(param_grid["min_samples_split"]) * len(param_grid["min_samples_leaf"]),
        cv,
    )
...
def run(cfg: dict[str, Any]) -> None:
    ...
    for name, train_fn in trainers:
        ...
        metrics = _evaluate(
            model, X_train, X_val, X_test, y_train, y_val, y_test, name, log_target=(log_target and not eval_log_space)
        )
    ...
    comparison_path.write_text(json.dumps({k: v for k, v in all_results.items()}, indent=2, default=str))
    ...
    val_summary = (
        ...
        .sort_values("r2", ascending=False)[["model", "r2", "mae", "rmse"]]
        .reset_index(drop=True)
    )
    logger.info("━━  Validation Leaderboard  ━━")
    for _, row in val_summary.iterrows():
        logger.info(
            "  %-22s  R²=%.4f  MAE=%s  RMSE=%s", row["model"], row["r2"], f"{row['mae']:,.0f}", f"{row['rmse']:,.0f}"
        )
