...
def numerical_correlations(df: pd.DataFrame) -> dict[str, float]:
    ...
    """
    num_cols = ["duration", "days_left", "departure_hour", "departure_month",
                "arrival_hour", "stopovers"]
    corrs = {}
    for col in num_cols:
...
def run(cfg: dict[str, Any]) -> None:
    ...
    airline_df = pd.DataFrame(insights["airline_pricing"])
    logger.info("Most expensive airline : %s  (BDT %s median)",
                airline_df.iloc[0]["airline"],
                f"{airline_df.iloc[0]['median_fare']:,.0f}")
    logger.info("Cheapest airline       : %s  (BDT %s median)",
                airline_df.iloc[-1]["airline"],
                f"{airline_df.iloc[-1]['median_fare']:,.0f}")

    for row in insights["seasonal_pricing"]:
        logger.info("Season %-17s → median BDT %s",
                    row["seasonality"], f"{row['median_fare']:,.0f}")

    logger.info("Booking window → fare:")
    for row in insights["days_left_buckets"]:
        logger.info("  %s days ahead → BDT %s (n=%d)",
                    row["booking_window"], f"{row['median_fare']:,.0f}", row["count"])
