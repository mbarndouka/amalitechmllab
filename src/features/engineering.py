...
def drop_redundant_columns(df: pd.DataFrame, cols: tuple[str, ...]) -> pd.DataFrame:
...
def engineer(df: pd.DataFrame, cfg: dict[str, Any]) -> FeatureSet:
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = split_train_val_test(X, y, test_size, val_size, random_state)
    ...
    X_train, X_val, X_test, scaler = fit_and_scale(X_train, X_val, X_test, numerical)
...
def run(cfg: dict[str, Any]) -> None:
    ...
    features_dir: str = cfg.get("data", {}).get("features_dir", "data/features")
