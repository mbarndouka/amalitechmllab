...
def drop_leakage_columns(df: pd.DataFrame, leakage: frozenset[str]) -> pd.DataFrame:
    to_drop = [c for c in df.columns if c in leakage]
    logger.info("Dropping leakage columns: %s", to_drop)
    return df.drop(columns=to_drop)


def drop_high_missingness_rows(df: pd.DataFrame, threshold: float) -> pd.DataFrame:
...
def parse_datetime_columns(df: pd.DataFrame, cols: tuple[str, ...]) -> pd.DataFrame:
...
def extract_temporal_features(df: pd.DataFrame, cols: tuple[str, ...]) -> pd.DataFrame:
    datetime_cols = [c for c in cols if c in df.columns and pd.api.types.is_datetime64_any_dtype(df[c])]
    new_features = {
        f"{col.replace('_datetime', '')}_{feat_name}": (getattr(df[col].dt, attr).astype("int8"))
...
def encode_stopovers(df: pd.DataFrame, col: str, ordinal_map: dict[str, int]) -> pd.DataFrame:
    if col not in df.columns:
        return df
    unknown = set(df[col].unique()) - set(ordinal_map)
    if unknown:
        raise ValueError(
            f"Unexpected values in '{col}': {unknown}. Expected one of {set(ordinal_map)}. Update STOPOVER_ORDINAL."
        )
...
def build_pipeline(cfg: dict[str, Any]) -> Transform:
    ...
    stopovers_order: list[str] = features_cfg.get("ordinal", {}).get("stopovers", list(STOPOVER_ORDINAL))
    ...
    return compose(
        ...
        partial(
            ...
            threshold=cleaning_cfg.get("missingness_drop_threshold", _DEFAULT_MISSINGNESS_THRESHOLD),
        ),
        partial(
            impute_missing,
            numerical_strategy=cleaning_cfg.get("numerical_impute_strategy", _DEFAULT_NUMERICAL_STRATEGY),
            categorical_fill=cleaning_cfg.get("categorical_impute_value", _DEFAULT_CATEGORICAL_FILL),
        ),
...
def clean(df: pd.DataFrame, cfg: dict[str, Any]) -> tuple[pd.DataFrame, CleaningReport]:
    ...
    temporal_names = tuple(
        f"{col.replace('_datetime', '')}_{feat}" for col in _DATETIME_COLUMNS for feat, _ in _TEMPORAL_ATTRS
    )
